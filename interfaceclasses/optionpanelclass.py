...
class OptionPanel:
    def __init__(self, manager, window_surface):
        ...
        box_label_rects = [
            (pos, (box_label_width, box_height)) for pos in box_label_pos
        ]

        box_entry_pos = [
            (box_label_width, (box_height + 1) * n - 2) for n in range(box_number)
        ]
        box_entry_width = OP_ENTRY_BOX_WIDTH
        box_entry_rects = [
            (pos, (box_entry_width, box_height)) for pos in box_entry_pos
        ]

        # box cpntainer
        self.pos = OP_PANEL_POS
        rect_size = (box_label_width + box_entry_width, (box_height + 1) * box_number)

        self.surface = pygame.Surface(rect_size)
        self.surface.fill(OP_PANEL_BACKGROUND_COLOR)
        rect = pygame.Rect(self.pos, rect_size)
        container = pygame_gui.core.ui_container.UIContainer(rect, manager)
        self.labels = []
